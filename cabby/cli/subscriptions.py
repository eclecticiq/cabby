...
def extend_arguments(parser):
    ...
    parser.add_argument("-a", "--action", dest="action", choices=ACTIONS,
    ...
    parser.add_argument("-s", "--subscription", dest="subscription_id",
            help="ID of an existing subscription")

    parser.add_argument("--count-only", dest="count_only", action='store_true',
...
def _runner(client, uri, args):
    ...
    if args.action in ONLY_TAXII_11 and args.version == TAXII_10:
        raise ValueError('Action "%s" is not supported in TAXII v%s' % (
            args.action, args.version))

    if args.action == 'subscribe':
        response = client.subscribe(args.collection_name,
                count_only=args.count_only, uri=uri)

    elif args.action == 'status':
        response = client.get_subscription_status(args.collection_name,
                subscription_id=args.subscription_id, uri=uri)

    elif args.action == 'pause':
        response = client.pause_subscription(args.collection_name,
                args.subscription_id, uri=uri)

    elif args.action == 'resume':
        response = client.resume_subscription(args.collection_name,
                args.subscription_id, uri=uri)

    elif args.action == 'unsubscribe':
        response = client.unsubscribe(args.collection_name,
                args.subscription_id, uri=uri)
